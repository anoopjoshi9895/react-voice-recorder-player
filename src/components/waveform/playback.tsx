...
function Playback() {
  ...
  const setUpCanvas = () => {
    ...
    unplayedCanvasRef.current = canvasElements?.find(
      (elem) => elem.id === "waveformgraph-unplayed-graph"
    ) as HTMLCanvasElement;
    playedCanvasRef.current = canvasElements?.find(
      (elem) => elem.id === "waveformgraph-played-graph"
    ) as HTMLCanvasElement;
    progressLineRef.current = canvasElements?.find(
  ...
  const playAudio = () => {
    ...
    if (audioELem.readyState !== 4) {
      const audioURL = window.URL.createObjectURL(
        new Blob([blob as BlobPart], { type: "audio/mpeg" })
      );
      ...
      if (audioELem.paused) {
        ...
        /* play audio again from the beginning (while it was already playing) */
  ...
  const adjustBarsIntoView = () => {
    ...
    const offset =
      (unplayedCanvasRef?.current?.offsetWidth ?? 1) /
      graphDataContext?.current.length;
  ...
  const drawPlayedBars = () => {
    ...
    function drawBar() {
      ...
      currentBar = Math.round(
        numberOfBars * ((audioRef?.current?.currentTime ?? 1) / durationUpdated)
      );
      if (
      ...
      if (+bar.x.toFixed(3) < (progressLineRef?.current?.offsetWidth ?? 1)) {
        progressBarCtx?.clearRect(
          0,
          0,
          progressLineRef?.current?.width ?? 1,
          progressLineRef?.current?.height ?? 1
        );
        progressBarCtx?.fillRect(
          bar.x,
          0,
          1,
          progressLineRef?.current?.offsetHeight ?? 1
        );
      }
    }
